	...
	if T.Retail then
        ...
        -- 修改后的核心代码
         if C.Maps.MinimapTracking then
             -- 经典版使用不同的对象层级
             local TrackingButton = IS_CLASSIC and MiniMapTracking or MiniMapTrackingButton
             ...
             if TrackingButton then
                 ...
                 -- 通用样式设置
                 TrackingButton:StripTextures()

                 -- 处理图标（经典版路径不同）
                 if TrackingButton.icon then
                     TrackingButton.icon:SetTexCoord(unpack(T.IconCoord))
                     TrackingButton.icon:SetSize(16, 16)
            
                      -- 经典版需要强制设置纹理
                     if IS_CLASSIC then
                         TrackingButton.icon:SetTexture("Interface\\Minimap\\Tracking\\None")
                     end
                 end

                -- 创建背景框（仅限可点击版本）
                 if not IS_CLASSIC then
                     TrackingButton:CreateBackdrop()
                     TrackingButton.Backdrop:SetOutside(TrackingButton.icon)
                 end
			end
		end
    end
